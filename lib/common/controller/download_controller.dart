...
class DownloadController extends GetxController {
  ...
  void resetConcurrency({int? gid}) {
    loggerSimple.d('重置并发设置开始: gid=$gid');

    // 记录当前正在运行的任务GID列表，用于后续恢复
    final List<int> runningTaskGids = gid != null
        ? [gid]
        : dState.galleryTasks
    ...
    // 取消网络请求
    if (gid != null) {
      final ct = dState.cancelTokenMap[gid];
      if (ct != null && !ct.isCancelled) {
        ct.cancel();
      }
      dState.cancelTokenMap.remove(gid);
    } else {
      // 取消所有网络请求（用于全局并发设置变更）
    ...
    for (int index = 0; index < galleryTask.fileCount; index++) {
      ...
      if (!waitedForShowKey && index > 0 && showKey == null) {
        ...
        try {
          ...
          logger.d('showKey等待超时，继续自足获取: gid=${galleryTask.gid}, index=$index');
        }
        logger.d(
